...
public class LayersSectionParser {
	...
	public void parse(PsdInputStream stream, int imageId) throws IOException {
		...
		if (length > 0) {
			...
			if (size > 0) {
				...
				for (int i = 0; i < layersCount; i++) {
					...
					layerParser.parse(stream, imageId);
				}

				for (LayerParser layerParser : parsers) {
					layerParser.parseImageSection(stream, imageId);
				}
			}
		...
		parseBaseLayer(stream, imageId);
	}

	private void parseBaseLayer(PsdInputStream stream, int imageId) throws IOException {
		...
		baseLayerParser.fireChannelsLoaded(channels, imageId);
	}
}
